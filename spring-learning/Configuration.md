...
从 Spring3.0 开始，`@Configuration` 可用于定义配置类，可替换 xml 配置文件，配置类内部包含有一个或多个被 @Bean 注解的方法，这些方法会被 AnnotationConfigApplicationContext 或 AnnotationConfigWebApplicationContext （web 环境）扫描，并用于构建bean定义、初始化Spring容器。
