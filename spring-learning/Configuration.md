...
从 Spring3.0 开始，`@Configuration` 可用于定义配置类，可替换 xml 配置文件，配置类内部包含有一个或多个被@Bean注解的方法，这些方法会被 AnnotationConfigApplicationContext 或 AnnotationConfigWebApplicationContext （web 环境）扫描，并用于构建bean定义、初始化Spring容器。

SpringBoot 中 web 环境下由 AnnotationConfigServletWebServerApplicationContext 处理配置类。
...
这里提及下 `@Configuration` 注解是原型注解 `@Component` 所标记的，即被它注解的类也会被自动检测（auto-detection）处理的。
...
```java
AnnotatedElementUtils.isAnnotated(SpringBootApplication.class, Configuration.class.getName());
```
...
SpringBootConfiguration 在 SpringBoot 中可以用来代替 Configuration，而这个注解在使用 SpringBootApplication 时会被自动带入。

SpringBootApplication 复合注解由如下几个注解构成：
```mermaid
graph TD
...
可以知道 @ComponentScan 注解同样会注册注解配置处理器的，再结合前文中提及 `@SpringBootApplication` 同时被 `@SpringBootConfiguration` 和 `@ComponentScan` 两个注解所标记，这就可以理解在使用 SpringBoot 的过程中，我们通常无需关心 ConfigurationClassPostProcessor 的配置了。
